-- ===========================================================================
-- monetization.schema.sql
-- SAMePOS monetization core — STANDALONE consolidated schema.
--
-- This is the same set of objects as db/migrations/025_monetization.sql,
-- packaged as a standalone schema you can apply to any PostgreSQL database
-- that already has the licensing core (db/schema/licensing.schema.sql):
--
--     psql -d <db> -f db/schema/licensing.schema.sql
--     psql -d <db> -f db/schema/monetization.schema.sql
--
-- It is idempotent and additive. It refuses to apply when the licensing core
-- (licence_node) is missing, because the monetization core builds on the
-- node's install-code identity.
--
-- Equivalence with the numbered migration is enforced by db/tests/run_db_tests.sh.
-- See db/README.md and docs/monetization.md for the model and integration points.
-- ===========================================================================
