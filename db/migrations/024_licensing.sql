-- ===========================================================================
-- 024_licensing.sql
-- SAMePOS licensing & trial-enforcement core.
--
-- PostgreSQL (developed and tested against PG 16; requires >= 11 for sha256()).
-- Idempotent and ADDITIVE: safe to run on a fresh node or an existing database.
-- It never drops or alters existing product tables. Applied inside one
-- transaction; if anything fails the whole migration rolls back.
--
-- Model (see the samepos-node-install runbook):
--   * One node == one venue box, identified by a machine-bound INSTALL CODE.
--   * Trial: a standard trial (14 days by default) auto-started on first boot
--     via licence_ensure_trial().
--   * Signed licences: Ed25519 "SPOS1." keys bound to the node's install code.
--     Ed25519 verification is performed by the FastAPI app using the vendor
--     PUBLIC key; the private signing key never leaves the vendor keystore.
--     This schema is the system-of-record + enforcement, not the verifier:
--     the app calls licence_record_signed() with the already-verified fields.
--   * Effective status (mode / validity / days remaining) is computed by
--     licence_effective_status() and exposed as the licence_status view — the
--     shape /api/licence/status should return.
--   * Enforcement is ADVISORY by default (the app reads status and decides).
--     An optional hard guard trigger blocks new sales when the licence is
--     invalid, gated by licence_config.hard_enforcement (shipped OFF).
--
-- App integration points:
--   first boot   -> licence_set_node(install_code[, app_version, hostname])
--                   then licence_ensure_trial()
--   /api/licence/status    -> SELECT * FROM licence_status;
--   /api/licence/activate  -> app verifies Ed25519, then
--                             licence_record_signed(mode, key, bound_code, expires, verified, ...)
