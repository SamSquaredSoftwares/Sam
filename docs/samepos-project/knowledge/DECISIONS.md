...
| 2026-09-12 | The cloud POS trading schema is designed as migration `db/cloud/migrations/0001_pos_trading.sql` (with rollback and disposable-cluster tests), NOT yet applied to the live Supabase project. | Design and verify first; applying to production is a separate deliberate step, and the sync engine's contract must be reconciled before first apply. | [repo] |
| 2026-09-12 | Cloud trading tables anchor on the existing `stores` table as the venue, carry the `(terminal_id, sale_ref)` unique constraint, and use composite FKs so a sale cannot reference another venue's terminal or business day. | Reuse the live registry rather than invent a second venue table; make idempotent re-sync and cross-venue integrity database facts, not app conventions. | [repo] |
| 2026-09-12 | Cloud RLS is SELECT-only for authenticated users, scoped by `pos_venue_members`; there are no client write policies. All cloud writes go through the service role (the sync engine). | Rule 6: trusted backend writers use the service role, clients get venue-scoped RLS. The cloud is a mirror, never the till. | [repo] |
| 2026-09-12 | `pos_sales.business_day_id` is nullable in the cloud even though the node requires it. | The node's NOT NULL on business_day_id is runbook gotcha #4. The mirror must accept late-arriving business days rather than lose sales; sync backfills. | [repo] |
| 2026-09-12 | The cloud stores `vat_cents` as pushed by the node and only bounds it (sign-aware, never above the total); it does not recompute the 15 percent split. | Node rounding is authoritative. A recompute-and-reject in the cloud would drop real sales over a one-cent rounding difference. | [repo] |
